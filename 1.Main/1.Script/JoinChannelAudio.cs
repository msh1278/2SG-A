...
using static Agora_RTC_Plugin.API_Example.Home;
...
namespace Agora_RTC_Plugin.API_Example.Examples.Basic.JoinChannelAudio
{
    public class JoinChannelAudio : MonoBehaviour
    {
        ...
        private void Start()
        {
            GameManager.Instance.voice = this;
            //LoadAssetData();

            //수정

            _appID = "f86b34d0a03c48979cd2535c603157ce";
            _channelName = SceneManager.GetActiveScene().name;

            DataBase.Instance.SendMessageApi("{\"channel\":\"" + SceneManager.GetActiveScene().name + "\",\"uid\":" + DataBase.Instance.stu_local_code + "}", "Token", (Success, request) =>
            {
                ...
                RtcEngine.MuteLocalAudioStream(true);//뮤트
                //RtcEngine.MuteLocalAudioStream(false);//뮤트 취소
            });
            PrepareAreaList();
            if (CheckAppId())
            {
        ...
        {
            RtcEngine.MuteLocalAudioStream(muteSet);//뮤트 취소
        }
